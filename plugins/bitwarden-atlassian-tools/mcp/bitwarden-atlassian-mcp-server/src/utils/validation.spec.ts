...
describe("validateInput", () => {
  it("should return validated data for valid input", () => {
    const result = validateInput(GetIssueSchema, {
      issueIdOrKey: "PROJ-1",
    });
...
describe("GetConfluencePageSchema", () => {
  ...
  it("should accept all bodyFormat variants", () => {
    for (const fmt of ["storage", "view", "export_view"]) {
      const result = GetConfluencePageSchema.parse({
        pageId: "1",
        bodyFormat: fmt,
      });
...
describe("GetConfluencePageCommentsSchema", () => {
  it("should reject empty pageId", () => {
    expect(() =>
      GetConfluencePageCommentsSchema.parse({ pageId: "" }),
    ).toThrow();
  });
  ...
  it("should accept storage bodyFormat", () => {
    expect(
      GetConfluencePageCommentsSchema.parse({
        pageId: "1",
        bodyFormat: "storage",
      }).bodyFormat,
    ).toBe("storage");
...
describe("GetIssueRemoteLinksSchema", () => {
  it("should accept valid issue keys", () => {
    const result = GetIssueRemoteLinksSchema.parse({
      issueIdOrKey: "PM-123",
    });
  ...
  it("should reject a non-atlassian.net hostname (SSRF)", () => {
    expect(() =>
      DownloadAttachmentSchema.parse({
        attachmentUrl: "https://attacker.com/secure/attachment/123/file.pdf",
      }),
    ).toThrow();
  });
