...
export class ConfluenceClient {
  ...
  private handleError(error: AxiosError): Error {
    if (error.response) {
      ...
      switch (status) {
        ...
        case 404:
          return new Error(
            `Confluence resource not found: ${error.config?.url}`,
          );
        case 429:
          return new Error(
            "Confluence API rate limit exceeded. Please try again later.",
          );
        default:
          return new Error(
    ...
    if (error.request) {
      return new Error(
        `Confluence API request failed: ${error.message}. Check your ATLASSIAN_CLOUD_ID.`,
      );
    }
  ...
  async getCommentReplies(
    params: ConfluenceGetCommentRepliesParams,
  ): Promise<ConfluenceCommentsResult> {
    const queryParams: Record<string, any> = {
      limit: Math.min(params.limit || 25, 100),
    };
  ...
  async getChildPages(
    pageId: string,
    limit: number = 25,
  ): Promise<ConfluenceChildPagesResult> {
  ...
  async listSpaces(
    limit: number = 25,
    type?: string,
  ): Promise<ConfluenceSpacesResult> {
    const queryParams: Record<string, any> = {
      limit: Math.min(limit, 250),
    };
   ...
   */
  async searchCql(
    params: ConfluenceCqlSearchParams,
  ): Promise<ConfluenceCqlSearchResult> {
    const queryParams: Record<string, any> = {
      cql: params.cql,
