...
import { validateInput, SearchConfluenceCqlSchema, SearchConfluenceCqlInput } from '../utils/validation.js';
  ...
  for (const item of results.results) {
    ...
    if (item._links?.webui) {
      const baseUrl = item._links.webui.startsWith('http')
        ? item._links.webui
        : `${process.env.CONFLUENCE_URL || process.env.JIRA_URL}${item._links.webui}`;
      output += `**URL:** ${baseUrl}\n`;
    }
