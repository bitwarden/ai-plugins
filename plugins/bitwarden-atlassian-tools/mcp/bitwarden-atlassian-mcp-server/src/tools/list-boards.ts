 ...
 */
function formatBoards(boards: JiraBoard[]): string {
  ...
  for (const board of boards) {
    ...
    if (board.location?.projectKey) {
      const projectName = board.location.projectName
        ...
        : "";
      output += `- Project: ${board.location.projectKey}${projectName}\n`;
    }
    output += `\n`;
  }
...
async function handler(input: any): Promise<string> {
  ...
  try {
    ...
    const response = await client.listBoards(
      validated.projectKeyOrId,
      maxResults,
    );

    return formatBoards(response.values);
  } catch (error) {
...
const listBoardsTool: ToolDefinition = {
  name: "list_boards",
  description:
    "List Jira Agile boards, optionally filtered by project. Returns board id, name, and type. Use a board id with get_sprints.",
  inputSchema: {
    type: "object",
    properties: {
      projectKeyOrId: {
        type: "string",
        description:
          'Filter boards by project key or numeric ID (e.g., "PM" or "10001")',
      },
      maxResults: {
        type: "number",
        description:
          "Maximum number of boards to return (default: 50, max: 100)",
        default: 50,
        minimum: 1,
        maximum: 100,
      },
