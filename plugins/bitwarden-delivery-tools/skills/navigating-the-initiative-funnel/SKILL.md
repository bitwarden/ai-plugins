...
**The Tech Breakdown is the canonical artifact for this phase.** Use `Skill(writing-tech-breakdowns)` for the end-to-end workflow — drafting, status lifecycle, stakeholder-communication checklist, cross-team signoff, and gate verification. The breakdown is what the shepherd reviews when "share it back" happens above.
...
- Related: `Skill(writing-tech-breakdowns)` for the team's Tech Breakdown coming out of Phase 4; `Skill(choosing-collaboration-model)` for picking a collaboration model on each cross-team impact identified during scoping or breakdown; `Skill(running-work-transitions)` for the Phase 4→5 transition mechanics on either side of the handoff; `Skill(architecting-solutions)` for the architectural judgment to bring to the breakdown.
