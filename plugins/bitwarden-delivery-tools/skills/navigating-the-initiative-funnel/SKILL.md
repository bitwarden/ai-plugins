...
- Related: [`bitwarden/tech-breakdowns`](https://github.com/bitwarden/tech-breakdowns) for the Tech Breakdown template and per-team folder conventions the team uses to draft the breakdown coming out of Phase 4; `Skill(running-work-transitions)` for the Phase 4→5 transition mechanics on either side of the handoff; `Skill(architecting-solutions)` for the architectural judgment to bring to the breakdown.
