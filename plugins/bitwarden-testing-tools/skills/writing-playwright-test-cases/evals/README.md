...
Every case's `prompt` embeds a full `## Application Context` section (`## States` and `## Flows`). `writing-playwright-test-cases`'s own `SKILL.md` requires this section as a precondition and instructs the skill to return an error and build nothing without one, so a prompt without it would not exercise the skill at all. Where a real state or flow already exists in `scoping-playwright-application-context`'s known-flows catalog (the trial-verification-email flow, the paid-org and free-user states), the context is drawn from that catalog rather than invented. Where no catalog entry exists (organization member management, two-step login OTP setup, a generic flag-gated feature), the context is a minimal synthetic state written to the same schema, noted as such.
