...
If a stop-and-report condition fires — the diff cannot be produced (step 1b), a repo cannot be mapped to a canonical name, or a change/route resolves to a service with no `services.md` entry — return a plain failure report naming the condition instead of a `<!-- SERVICES START -->` artifact. Do not emit a services fence assembled from partial or guessed data.
