...
allowed-tools: Read, Bash(${CLAUDE_SKILL_DIR}/scripts/read_mailcatcher.py:*), Bash(${CLAUDE_SKILL_DIR}/scripts/get_admin_email.py:*)
---
...
| 1    | matched a URL, but its host is not on the local dev allowlist (no retry)           | stderr says _not a local dev host_; add the hostname to `PLAYWRIGHT_TESTING_ALLOWED_HOSTS` (see Prerequisites) rather than widening `--link-filter` |
...
A second granted helper, `${CLAUDE_SKILL_DIR}/scripts/get_admin_email.py`, prints the Bitwarden dev admin address (read only from `adminSettings.admins` in your `bitwarden/server` checkout's `dev/secrets.json`). Use it to supply `--recipient` for the Admin Portal magic-link flow; see `${CLAUDE_SKILL_DIR}/references/email-patterns.md` for the exact invocation. It reads `server/dev/secrets.json` **relative to the current working directory**, so run it from the workspace root that holds your `bitwarden/server` checkout, or point it elsewhere with `--secrets-file <path>`. It exits 3 when that file is missing, unreadable, or has no `adminSettings.admins` key; the stderr message names the path it tried.
