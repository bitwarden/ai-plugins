...
| 1    | `NO_MATCH: no email for recipient …` — no matching message arrived (the script retried once after a 3s sleep)                      | if the email may not have arrived yet (a slow async send), re-invoke the script once more — a single fresh call, not a loop; the script's own 3-second retry supplies the delay. Otherwise the filters are too narrow: re-check `--recipient`, then widen or drop `--pattern`.                                                 |
| 1    | matched a message, but no URL matched `--link-filter` (no retry; stderr: `matched but contained no URL filtered by …`)             | the link is there but the filter missed it; loosen or fix `--link-filter`                                                                                                                                                                                                                                                      |
| 1    | matched URLs, but none is on the local dev allowlist (no retry; stderr: `none of the N filter-matched URL(s) is a local dev host`) | stderr names the rejected host(s). This is an environment change outside the skill: ask the user to add the host to `MAILCATCHER_ALLOWED_HOSTS` in their shell (see Prerequisites), then retry. Do not prefix the assignment onto the script invocation — that falls outside the skill's Bash grant and re-prompts every call. |
| 2    | usage error (bad or missing arguments)                                                                                             | fix the invocation                                                                                                                                                                                                                                                                                                             |
| 3    | Mailcatcher unreachable or returned invalid JSON                                                                                   | Mailcatcher is not running or unreachable. Starting it falls outside this skill's Bash grant: ask the user to start it (see Prerequisites), then retry.                                                                                                                                                                        |
...
A second granted helper, `${CLAUDE_PLUGIN_ROOT}/skills/reading-mailcatcher-api/scripts/get_admin_email.py`, prints the Bitwarden dev admin address (read only from `adminSettings.admins` in your `bitwarden/server` checkout's `dev/secrets.json`). It defaults to `server/dev/secrets.json` relative to the current directory, so run it from the workspace root that contains the `server` checkout, or pass `--secrets-file <path>`. Use it to supply `--recipient` for the Admin Portal magic-link flow. Its exit codes differ from the table above: 2 = usage error; 3 = the secrets file is missing, unreadable, invalid JSON, or has no `adminSettings.admins` key. See `${CLAUDE_PLUGIN_ROOT}/skills/reading-mailcatcher-api/references/email-patterns.md` for the exact invocation, the `--secrets-file` override, and the `--all` flag.
...
The script always reads Mailcatcher at `http://localhost:1080`, the fixed endpoint across Bitwarden dev environments; there is no URL override. Extracted URLs are restricted to a local dev allowlist (`localhost`, `127.0.0.1`, `::1`, `bitwarden.test`). If your environment's emails link to another local hostname, it must be added to the comma-separated `MAILCATCHER_ALLOWED_HOSTS` env var, which extends (never replaces) that allowlist. This is an environment change the user makes in their shell, not something to prefix onto the script invocation: an assignment prepended to the command falls outside the skill's `script:*` Bash grant, so it re-prompts on every call and does not persist.
