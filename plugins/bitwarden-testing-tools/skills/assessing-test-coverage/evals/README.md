...
- `baseline.json` — last known-good run. Diff against this to spot regressions on future description changes. Recorded 2026-09-11 with `--model claude-opus-4-8` at `--runs-per-query 7`, and records the run's `model`, `runs_per_query`, and `recorded_utc`, plus per-query `timeouts` and `first_skills`.
