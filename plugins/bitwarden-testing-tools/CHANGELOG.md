...
- Applied two non-blocking items from the local validation pass: tightened the `reading-mailcatcher-api` `--link-filter` default wording, and set `assessing-test-coverage/evals/run_real_eval.py` to mode `100755` so all three eval-runner wrappers are executable and uniform.
