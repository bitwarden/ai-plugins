...
- Two execution-phase agents: `localhost-web-health-checker`, which gates the run on environment health, and `playwright-test-runner`, which executes the plan and returns the segment results JSON. Both carry the per-run `UNTRUSTED-SOURCE-<nonce>` guardrail, so source quoted into the artifacts they read stays data, not instructions.
