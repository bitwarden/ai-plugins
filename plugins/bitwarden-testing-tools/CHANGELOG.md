...
- `assessing-test-coverage` eval harness brought in step with the new skill's harness: the read-only `gh`/`git` allowlist now guards its `gh api repos/` carve-out against write requests (any method or request-body flag bails, across spaced, joined, and `=`-attached spellings) and against process-substitution and redirection operators, and `baseline.json` was re-recorded on `claude-opus-4-8` at 7 runs/query, now persisting the run's `model`, `runs_per_query`, and `recorded_utc` plus per-query `timeouts`, `bails`, and `first_skills`.
- Both eval harnesses now detect shell chains with a quote-aware `shlex` tokenizer instead of a raw substring scan, so a redirect-looking character inside a quoted argument (`gh search code "Task<Cipher>"`) no longer misfires the `<`/`>` guard and mis-records a legitimate read-only lookup as a real-work bail; a bail is also tracked as its own per-query `bails` count so it is distinguishable from a clean non-trigger during triage. Both `baseline.json` files were re-recorded on `claude-opus-4-8` at 7 runs/query (10/10 should-trigger, 10/10 should-not-trigger, no verdict changes).
- Both eval harnesses no longer over-bail three classes of genuinely read-only lookup: command substitution and backticks are now detected quote-aware (a single-quoted `'$(...)'` literal is left alone, while a double-quoted `"$(...)"` still bails), a clustered short flag such as `-sf` is no longer misread as the write flag `-f` (only a short flag's leading letter is checked, since every write short flag takes a value and must lead its cluster), and the `gh api` carve-out again covers `gh api graphql`, `gh api /repos/...`, and `gh api user` rather than only `gh api repos/` (the method/body flag guard, not the prefix, is what enforces the read).
