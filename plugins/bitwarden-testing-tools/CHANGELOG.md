...
- Corrected every skill's Bash tool grant from the non-matching `Bash(<script> *)` form to the working `Bash(<script>:*)` prefix form, and added the `Read`/`Grep`/`Glob` tool grants the skills rely on so their reference-file reads are auto-approved rather than prompting on every open.
