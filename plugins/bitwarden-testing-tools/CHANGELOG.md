...
- Consolidated the per-skill eval runners into a single shared engine at `evals/run_real_eval.py`. The `assessing-test-coverage` and `recommending-test-layers` skills previously each shipped a byte-identical copy of the runner that differed only in a hardcoded skill token, so every engine fix had to be hand-applied to both. The shared runner resolves the target skill token from `--skill`, or infers it from the eval-set path when the flag is omitted, so a skill's `evals/` directory now holds only its data (`trigger-eval.json`, `baseline.json`). Adding evals for a new skill no longer requires copying the engine.
- Consolidated the three eval READMEs into the single shared `evals/README.md`. The two per-skill READMEs duplicated the running, regression-check, and test-surface-maintenance instructions verbatim, so each procedural change had to be applied twice; they are removed and their unique content (the known below-threshold queries) folded into the shared README, leaving procedural docs to maintain in one place.
...
- Restored the shared runner's timeout accounting, dropped in the consolidation. A per-run `timed_out` flag (kept out of the persisted `baseline.json`) and a per-query stderr warning now distinguish a slow, timed-out run from a genuine non-trigger, so a slow should-trigger query is not silently read as a real failure.
- Recorded the model each `baseline.json` was captured on in a top-level `model` field, so a re-run can match it; the shared `evals/README.md` `--model` guidance previously told runners to match a model no baseline recorded.
- Corrected the eval READMEs to describe the plugin-qualified trigger detection (`<plugin>:<skill>` Skill invocation or a `Read` of the skill's own `SKILL.md`, never a bare token substring) rather than the pre-fix bare-token wording, and documented the `--plugin` flag in the shared runner's Arguments list.
- Re-recorded both `baseline.json` files with the tightened runner (and, for `recommending-test-layers`, its tightened description; `assessing-test-coverage`'s description is unchanged); the previous baselines were recorded with the looser detection, so this supersedes the 1.2.0 baselines. `recommending-test-layers` now passes 10/10 should-trigger and 10/10 should-not-trigger. `assessing-test-coverage` passes 9/10 should-trigger and 10/10 should-not-trigger: its `does the autofill refactor PR ... which layers do they land at` query recovered from below threshold to a pass now that the sibling no longer cannibalizes it, leaving `audit the current test coverage on the feat/cipher-key-rotation branch` (2/7) as the single recorded below-threshold should-trigger query, documented in the shared `evals/README.md`.
