...
- `scripts/repo-diff.sh`, a small wrapper the scoping and mapping skills call to list a repo's changed files (`git diff --name-only origin/main...HEAD`), so their Bash grant is scoped to the script rather than to `git`.
...
- Behavior evals for `scoping-playwright-application-context`, six advice-only cases. The suite is kept as an authoring aid and has not been benchmarked.
