...
| `recommending-test-layers`  | Recommends **which tests to add and at which layer**. From a Jira key, Testmo CSV, an `assessing-test-coverage` report, a PR, or a feature description, places each behavior at its lowest sufficient deterministic layer (static / unit / component / contract) so the pre-merge gate stays fast and fully controlled, then adds a non-deterministic post-deploy layer where its trigger is met: smoke and E2E for the most critical journeys (graded against Bitwarden's Severity guide), integration for a doubled external boundary, synthetic monitoring for an SLO-bound journey, exploratory for irreducibly uncertain behavior. Writes a markdown recommendation report under `${CLAUDE_PLUGIN_DATA}/recommending-test-layers/`. |
