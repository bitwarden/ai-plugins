...
| `recommending-test-layers`  | Recommends **which tests to add and at which layer**. From a Jira key, Testmo CSV, an `assessing-test-coverage` report, a PR, or a feature description, places each behavior at its lowest sufficient deterministic layer (static / unit / component / contract) so the pre-merge gate stays fast and fully controlled, then grades criticality against Bitwarden's Severity guide to decide which critical journeys additionally earn a post-deploy layer (E2E / smoke / integration / synthetic monitoring / exploratory) — writing a markdown recommendation report under `${CLAUDE_PLUGIN_DATA}/recommending-test-layers/`. |
