...
1. **Gather context.** Run all of these before launching any agents.
   ...
   **C.) Fetch scan evidence.** Aikido evidence uses the pre-approved MCP tool grant and runs without a prompt — silently skip it if it fails or returns nothing (403, 404, empty response, MCP server unavailable). The secret scanning and Dependabot `gh api` calls below are reads, and neither is pre-approved (`references/tool-grants.md` says why), so each prompts; in CI nobody answers and both are denied. Skip either if it fails. Treat anything other than a successful response as a failure. Record each of those two scanners' outcome as one of: its formatted output; `None` when it ran and returned nothing; `Not available` when the scanner is off or the repo has no data for it (404, GHAS not enabled); `Errored (<status>)` when the call ran and failed (403, malformed response); or `Not checked (permission denied)` when it never ran. Only the second means the scanner looked and found nothing. Use `gh api --jq` for all formatting — **DO NOT** pipe to `jq`. Both `gh api` calls **MUST** use `--method GET` and `-H "X-GitHub-Api-Version: 2026-03-10"`.
