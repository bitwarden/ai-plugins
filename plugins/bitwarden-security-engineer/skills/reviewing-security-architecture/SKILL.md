...
Before evaluating a design, check it against Bitwarden's Architecture Decision Records by invoking `Skill(bitwarden-adr-tools:consulting-adrs)` and passing it the design itself, including the domain it touches and the specific elements at stake (trust boundaries, auth patterns, data stores, contracts). It returns conflict / gap / stale-reference / aligned findings with cited ADRs. Applied to an architecture review specifically:
