...
Before evaluating a design, check it against Bitwarden's Architecture Decision Records by invoking `Skill(bitwarden-adr-tools:consulting-adrs)` — it returns conflict / gap / stale-reference / aligned findings with cited ADRs. Applied to an architecture review specifically:
