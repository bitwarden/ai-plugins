...
Bitwarden's accepted architecture decisions are catalogued separately from the security principles above. To check a design or change against them, invoke `Skill(bitwarden-adr-tools:consulting-adrs)` — it returns structured findings (conflict, gap, stale-reference, aligned) with cited ADRs.
