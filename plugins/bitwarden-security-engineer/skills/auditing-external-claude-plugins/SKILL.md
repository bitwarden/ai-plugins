...
1. Run `${CLAUDE_SKILL_DIR}/scripts/gather-evidence.sh $plugin-repo-url $commit-sha`. It clones the repo, checks out the commit, and, if `.mcp.json` launches a pinned npm package, pulls that package's registry metadata, tarball, integrity hash, attestations, and audit data. The last line of its output is the scratch directory holding everything it gathered. Symlinks in the clone and in the extracted tarball are neutralized before you read anything: committed ones become plain files holding their target path, packaged ones are deleted, and both are inventoried in `symlinks.txt` (repo) and `pkg/symlinks.txt` (tarball). If it reports `NO_NPM_PACKAGE_DETECTED`, check `plugin.json`'s `mcpServers` field and the server's own dependency manifest manually; the script only covers the single-pinned-npm-package case. Every other marker it prints on stderr names evidence it could not collect; carry each one into the report's `**Not done:**` field rather than treating the absence as a clean result.
