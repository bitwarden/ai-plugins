...
| `auditing-external-claude-plugins` | Security audit of a third-party Claude Code plugin at a pinned commit before it's vendored. Covers MCP server config, dependency supply chain, tool permission scope, and prompt-injection surface. Runs as an isolated subagent and writes a report file, whether triggered manually or by a CI workflow that posts it as a PR comment. |
