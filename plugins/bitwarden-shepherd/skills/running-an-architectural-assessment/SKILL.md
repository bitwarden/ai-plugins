...
Phase 2 of the [Software Initiative Funnel](https://bitwarden.atlassian.net/wiki/spaces/EN/pages/584515614) is where the shepherd converts a documented problem into a documented direction. The deliverable is an **Architectural Assessment** — a Confluence page in the EN-space assessments folder that captures the refined problem statement, the current state, 2–4 evaluated solution options, and a recommendation with rationale.

Time budget: 3–5 weeks, 40–80 hours of shepherd time. The phase splits roughly in half — Part A (~2 weeks) understanding the problem, Part B (~2 weeks) finding the right direction.
