...
description: Peer-Reviewer and portfolio-curator side of the TSI Shepherding Model. Covers serving as constructive challenge function for someone else's Technical Strategy Idea, stewarding the backlog (weekly triage, monthly RICE updates, Now/Next/Later placement), the quarterly prioritization review with engineering leadership, and the handoff of approved ideas to the Software Initiative Funnel at Identification.
when_to_use: Use when you are assigned as peer reviewer on someone else's TSI, when you are stewarding the broader ARCH idea portfolio, when preparing the quarterly prioritization review presentation, or when transitioning an approved idea to the funnel. Trigger phrases include "I'm peer reviewer on ARCH-…", "peer-reviewing a TSI", "Architecture Council prep", "quarterly RICE scoring", "Now/Next/Later placement", "transitioning ARCH-X to the funnel". Do not invoke when you are the named Primary Owner driving a specific idea (use `championing-a-strategy-idea`) or for the team-contributor framing of recognizing patterns worth filing from inside a team (use `contributing-to-technical-strategy` in `bitwarden-tech-lead`).
...
Peer-Reviewer and portfolio-curator playbook for Bitwarden's [Technical Strategy Ideas](https://bitwarden.atlassian.net/wiki/spaces/EN/pages/2344517656) (TSI) backlog — the upstream idea-stage system that feeds the Software Initiative Funnel at Identification.
