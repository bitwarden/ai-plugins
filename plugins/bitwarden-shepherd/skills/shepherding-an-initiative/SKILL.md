...
description: End-to-end umbrella playbook for shepherding one cross-cutting initiative through all five phases of Bitwarden's Software Initiative Funnel — Identification, Research, Proof of Concept, Scoping & Commitment, Implementation. Maps each phase's effort, deliverables, and decision gate; holds the line on what the shepherd produces vs. what gets handed to teams; dispatches to the phase-deep shepherd skills at the right moment.
when_to_use: Use when an approved ARCH idea is entering the funnel, when the shepherd needs to choose which phase deep-dive to invoke, when preparing for a phase decision gate, or when reasoning about the shepherd vs. team ownership boundary across phases. Trigger phrases include "ARCH-X just got approved", "I'm assigned shepherd on X", "what phase am I in", "what's the next gate". Do not invoke for the pre-funnel championing arc before an ARCH idea is approved at quarterly prioritization (use `championing-a-strategy-idea`) or for peer-reviewer / portfolio-curator work on the upstream backlog (use `curating-the-strategy-ideas-backlog`).
...
Five-phase umbrella playbook for an initiative shepherd. Fetch the [Software Initiative Funnel](https://bitwarden.atlassian.net/wiki/spaces/EN/pages/584515614) page via `get_confluence_page` when entry/exit criteria or full template detail is needed.
