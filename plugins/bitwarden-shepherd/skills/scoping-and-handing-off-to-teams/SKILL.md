...
description: Phase 4 (Scoping & Commitment) deep dive for an initiative shepherd. Covers writing the High-Level Architecture Plan, creating child epics, running the per-team handoff meeting, holding the line against shepherd-written stories, cost/benefit analysis, presenting for leadership commitment, and operational prioritization with capacity allocation. Composes the originating side of the Work Transition Playbook. Use when an initiative has cleared PoC and is being scoped, handed off to teams, and committed for Implementation.
...
Phase 4 of the [Software Initiative Funnel](https://bitwarden.atlassian.net/wiki/spaces/EN/pages/584515614) is the final decision gate before significant resource allocation. The shepherd transitions from leading the work to supporting the teams who will execute it. This phase is also when the Phase 4 → Phase 5 handoff begins — a [Work Transition Playbook](https://bitwarden.atlassian.net/wiki/spaces/EN/pages/2521038855) transition from the originating side. Compose `Skill(running-work-transitions)` in `bitwarden-delivery-tools` for the transition mechanics; the originating-side guidance there (Phases 1–2 of the playbook — Preparation and Transition Sessions) is exactly what's happening when you produce the architecture plan, create epics, and run handoff meetings.

Time budget: 2–4 weeks, 30–50 hours of shepherd time. Two activities dominate: producing the High-Level Architecture Plan, and the per-team handoff meetings.
