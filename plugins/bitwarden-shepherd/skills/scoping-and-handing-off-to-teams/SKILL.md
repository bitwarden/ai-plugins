...
description: Phase 4 (Scoping & Commitment) deep dive for an initiative shepherd. Covers writing the High-Level Architecture Plan, creating child epics, running the per-team handoff meeting, holding the line against shepherd-written stories, cost/benefit analysis, presenting for leadership commitment, operational prioritization, and capacity allocation. Composes `Skill(running-work-transitions)` in `bitwarden-delivery-tools` for the originating-side mechanics of the Phase 4→5 handoff. Time budget: 2–4 weeks, 30–50 hours.
when_to_use: Use when an initiative has cleared PoC at Architecture Council and the shepherd is scoping the work, drafting the High-Level Architecture Plan, creating child epics, running per-team handoff meetings, and seeking the leadership go/no-go for Implementation. Trigger phrases include "handoff meeting", "drafting the High-Level Architecture Plan", "writing child epics", "cost-benefit analysis", "go/no-go presentation", "capacity allocation". Do not invoke for ongoing cross-team coordination after Implementation begins (Phase 5 — use `coordinating-implementation-across-teams`) or for writing the teams' stories yourself — the team owns the breakdown.
...
Phase 4 (Scoping & Commitment) deep-dive playbook for an initiative shepherd. Composes `Skill(running-work-transitions)` in `bitwarden-delivery-tools` for the originating-side Preparation and Transition Sessions phases of the [Work Transition Playbook](https://bitwarden.atlassian.net/wiki/spaces/EN/pages/2521038855).
