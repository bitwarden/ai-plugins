...
Write to `${CLAUDE_PLUGIN_DATA}/coverage-reports/<slug>-<timestamp>-coverage.md`:

- `<slug>` — a kebab-case slug for the change (from the ticket key, PR number, or feature name).
- `<timestamp>` — `YYYY-MM-DD-HHMMSS`, from `Bash(date +%Y-%m-%d-%H%M%S)`.
...
A top `# <title> — <change>` heading, then a one-line meta line (`ticket/PR · status · team · timestamp`),
