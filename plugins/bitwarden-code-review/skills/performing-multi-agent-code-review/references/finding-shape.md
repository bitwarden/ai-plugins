...
| field          | type    | notes                                                                                                 |
| -------------- | ------- | ----------------------------------------------------------------------------------------------------- |
| `id`           | string  | `{source}-{n}`, e.g. `"bug-3"`. Source ∈ `arch`, `quality`, `bug`, `sec`, `cfg`, `skl`, `val`.        |
...
| `source_agent` | string  | `"architect"` \| `"quality"` \| `"bug"` \| `"security"` \| `"config"` \| `"skill"` \| `"validation"`. |

If an agent produces no findings, return `[]`.

The orchestrator renders `source_agent` on every finding in the final report — set it accurately. The id-prefix → source_agent mapping is fixed: `arch → architect`, `quality → quality`, `bug → bug`, `sec → security`, `cfg → config`, `skl → skill`, `val → validation`.
