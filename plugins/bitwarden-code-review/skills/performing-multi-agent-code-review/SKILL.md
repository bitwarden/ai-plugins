...
**One carve-out: Agent 5.** `plugin-dev:skill-reviewer` declares `tools: ["Read", "Grep", "Glob"]`, so it cannot invoke `Skill(bitwarden-security-engineer:bitwarden-security-context)` and cannot run the `gh`/`git` commands Tool Discipline governs. It receives Untrusted Input Boundary and Line Number Accuracy, and nothing else from the bundle. This is safe only because it never classifies severity or reasons about vault data: it returns a prose report, and the orchestrator does the classification in Step 3. Do not extend the carve-out to any agent that emits Finding Shape objects directly.
...
3. Send all Agent tool calls for this step in a single message (**DO NOT** use run_in_background because the agents must run synchronously to guarantee findings are validated together at Step 4). Launch the 3 agents below — plus a conditional 4th (Agent 4) when Claude configuration files **or** changed `SKILL.md` files were detected in Step 1 and the `claude-config-validator` plugin is installed, and a conditional 5th (Agent 5) when changed `SKILL.md` files were detected and the `plugin-dev` plugin is installed. A changeset of nothing but `SKILL.md` launches both. Agents 1–2 and the conditional Agents 4–5 use the resolved analysis model, Agent 3 uses the resolved security model (see Model Selection). Agents 1–4 each receive the diff and the Review Rules, and each emits findings as a JSON array per the Finding Shape schema; Agent 5 is the exception on both counts, per the Review Rules carve-out and its own paragraph below. Confidence Scoring from `references/evaluation-standards.md` applies to all findings — threshold ≥ 80. In PR mode, pass the PR title and description only to Agent 3 per Context Partitioning — Agents 1, 2, 4, and 5 receive diff + Review Rules only.
   ...
   **Agent 5 (conditional): Skill review agent**
   ...
   Use the `plugin-dev:skill-reviewer` subagent type with the resolved analysis model (see Model Selection), scoped to the changed `SKILL.md` files, to review frontmatter, description trigger quality, content length, writing style, progressive disclosure, and referenced files that do not exist. Pass only the carve-out subset of the Review Rules.
   ...
   **This agent does not emit Finding Shape objects.** Its system prompt fixes its output as a prose report with `#### Critical` / `#### Major` / `#### Minor` sections, so instructing it to return JSON puts two output contracts in one context and the schema is not the one that wins. Take its report as-is and translate it yourself, here in Step 3, before Step 4 runs:
   - One Finding object per entry under its `Critical`, `Major`, and `Minor` headings. Ignore `Positive Aspects`, `Overall Rating`, and `Priority Recommendations` — praise and summary are not findings.
   - Map `Critical → blocker`, `Major → important`, `Minor → refactor`.
