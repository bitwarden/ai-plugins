...
- Conversation logs from `${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/logs/`
...
2. **Data Collection** - The skill will:
   ...
   - Parse conversation logs from `${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/logs/`
...
```
${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/reports/YYYY-MM-DD-description-SESSION_ID.md
```

Example:
```
${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/reports/2025-10-23-authentication-refactor-3be2bbaf.md
```
...
├── logs/
│   ├── *.md                    # Conversation logs (markdown format)
│   └── *.ndjson                # Conversation logs (structured format)
└── scripts/
    └── analyze-logs.sh         # Log analysis helper (future)
```
...
```
${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/patterns/
...
- Logs are generated by hooks in `${CLAUDE_PLUGIN_ROOT}/hooks/`
- Check if hooks are enabled and properly configured
- You can still do a retrospective using git history + your feedback
...
- Conversation logs stored in `${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/logs/`
...
- Reports stored in your repository (`${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/reports/`)
...
- Modify `${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/SKILL.md` (Claude's instructions)
