...
LOGS_DIR="${PLUGIN_ROOT}/skills/retrospecting/logs"
...
if [ -x "$HOOKS_DIR/logging_utils.py" ]; then
...
echo "$TEST_INPUT" | "$HOOKS_DIR/SessionStart"
if [ $? -eq 0 ]; then
    echo "  ✓ SessionStart executed successfully"

    # Check if log files were created
    LOG_COUNT=$(find "$LOGS_DIR" -name "*test-session*.ndjson" 2>/dev/null | wc -l)
    if [ "$LOG_COUNT" -gt 0 ]; then
        echo "  ✓ NDJSON log file created"
        NDJSON_LOG=$(find "$LOGS_DIR" -name "*test-session*.ndjson" | head -1)
        echo "  📄 Log: $NDJSON_LOG"
    ...
    MD_COUNT=$(find "$LOGS_DIR" -name "*test-session*.md" 2>/dev/null | wc -l)
...
echo "$TEST_INPUT" | "$HOOKS_DIR/UserPromptSubmit"
if [ $? -eq 0 ]; then
    echo "  ✓ UserPromptSubmit executed successfully"

    # Check all test-session logs for the event
    if cat "$LOGS_DIR"/*test-session*.ndjson 2>/dev/null | grep -q '"e":"up"'; then
...
echo "$TEST_INPUT" | "$HOOKS_DIR/PostToolUse"
if [ $? -eq 0 ]; then
    echo "  ✓ PostToolUse executed successfully"

    if cat "$LOGS_DIR"/*test-session*.ndjson 2>/dev/null | grep -q '"e":"tu"'; then
...
echo "$TEST_INPUT" | "$HOOKS_DIR/SubagentStop"
if [ $? -eq 0 ]; then
    echo "  ✓ SubagentStop executed successfully"

    if cat "$LOGS_DIR"/*test-session*.ndjson 2>/dev/null | grep -q '"e":"ss"'; then
...
echo "$TEST_INPUT" | "$HOOKS_DIR/SessionEnd"
if [ $? -eq 0 ]; then
    echo "  ✓ SessionEnd executed successfully"

    if cat "$LOGS_DIR"/*test-session*.ndjson 2>/dev/null | grep -q '"e":"end"'; then
...
echo "  --------------------------"
cat "$LOGS_DIR"/*test-session*.ndjson | while read line; do
    echo "  $line"
done
echo ""
...
echo "  -------------------------------------"
MD_LOG=$(find "$LOGS_DIR" -name "*test-session*.md" | head -1)
head -20 "$MD_LOG" | sed 's/^/  /'
echo "  ..."
echo ""

# Count events across all files
EVENT_COUNT=$(cat "$LOGS_DIR"/*test-session*.ndjson | wc -l)
...
INVALID=0
cat "$LOGS_DIR"/*test-session*.ndjson | while IFS= read -r line; do
