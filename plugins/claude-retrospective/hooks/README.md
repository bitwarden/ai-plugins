...
These hooks automatically log all session activity to `${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/logs/` in two formats:
...
Logs are stored in `${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/logs/` with filename format:
```
...
find ${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/logs -name "*.ndjson" -mtime +30 -delete
find ${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/logs -name "*.md" -mtime +30 -delete
```
...
```bash
chmod -x ${CLAUDE_PLUGIN_ROOT}/hooks/SessionStart
chmod -x ${CLAUDE_PLUGIN_ROOT}/hooks/UserPromptSubmit
chmod -x ${CLAUDE_PLUGIN_ROOT}/hooks/PostToolUse
chmod -x ${CLAUDE_PLUGIN_ROOT}/hooks/Stop
chmod -x ${CLAUDE_PLUGIN_ROOT}/hooks/SessionEnd
chmod -x ${CLAUDE_PLUGIN_ROOT}/hooks/SubagentStop
```
...
- Logs are stored locally in `${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/logs/`
- Add `${CLAUDE_PLUGIN_ROOT}/skills/retrospecting/logs/` to `.gitignore` if sensitive data is involved
